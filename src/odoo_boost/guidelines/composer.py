...
def compose_agent_guidelines(version: str | None, reference_dir: str) -> str:
    ...
    lines = [
        ...
        "",
        "Start at the named file or symbol and make the smallest correct change.",
        "For a clear local edit that preserves access, stored data meaning, and",
        "cross-model behavior: locate the target, edit, run the cheapest relevant",
        "check, review the diff, and stop. Handle routine edits directly.",
        "Widen only for a named dependency, ambiguous target, failed check, or risk.",
        "",
        "## Version and execution boundary",
        "",
        version_guidance(version),
        "Verify a version fact against the target source or version note only when",
        "the change depends on that fact.",
        ...
        "",
        "Low (default): clear local edits, including labels and PO translations.",
        "Medium: bounded new fields, actions, smart buttons, or uncertain inheritance;",
        "check the affected model, view, action, and access boundary. High:",
        ...
        "Use specialist tracing or review only for a named uncertainty or risk.",
        ...
        "Open a skill when it supplies a procedure needed for this change.",
