...
class Order(models.Model):
    ...
    def _onchange_line_ids(self):
        ...
        self.amount_total = sum(line.quantity * line.price_unit for line in self.line_ids)
```
...
def _onchange_category_id(self):
    return {
        "domain": {
            "product_id": [("categ_id", "=", self.category_id.id)] if self.category_id else [],
        }
    }
```
...
def _onchange_field_name(self):
    _logger.info(
        "Onchange triggered: field=%s, value=%s, self.id=%s", "field_name", self.field_name, self.id
    )
