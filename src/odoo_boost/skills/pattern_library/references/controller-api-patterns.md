<!-- Adapted from letzdoo/claude-marketplace. Examples are references, not active-version facts. Odoo Boost version guidance and the target source take precedence. -->
...
def search(self, query="", limit=10, **kw):
    records = request.env["my.model"].search([("name", "ilike", query)], limit=int(limit))
...
def list_records(self, **kw):
    ...
    records = request.env["my.model"].search([], limit=limit, offset=offset)
    ...
    return request.make_response(
        json.dumps({"data": data}), headers=[("Content-Type", "application/json")]
    )
```

### POST Endpoint
```python
@http.route("/api/v1/records", type="http", auth="user", methods=["POST"], csrf=False)
...
def json_list_records(self, domain=None, fields=None, limit=100):
    ...
    records = request.env["my.model"].search_read(domain, fields, limit=limit)
...
class MyAPIController(http.Controller):
    ...
    # LIST
    @http.route("/api/v1/mymodel", type="http", auth="user", methods=["GET"], csrf=False)
    def list(self, **kw):
    ...
    # GET
    @http.route("/api/v1/mymodel/<int:id>", type="http", auth="user", methods=["GET"], csrf=False)
    def get(self, id):
    ...
    # CREATE
    @http.route("/api/v1/mymodel", type="http", auth="user", methods=["POST"], csrf=False)
    ...
    # UPDATE
    @http.route(
        "/api/v1/mymodel/<int:id>", type="http", auth="user", methods=["PUT", "PATCH"], csrf=False
    )
    ...
    # DELETE
    @http.route(
        "/api/v1/mymodel/<int:id>", type="http", auth="user", methods=["DELETE"], csrf=False
    )
    def delete(self, id):
...
class WebhookController(http.Controller):
    @http.route("/webhook/my_module", type="json", auth="none", methods=["POST"], csrf=False)
    ...
    def _verify_signature(self, data, signature, secret):
        ...
        if not signature or not secret:
            return False
        expected = hmac.new(secret.encode(), json.dumps(data).encode(), hashlib.sha256).hexdigest()
...
### File Upload
```python
@http.route("/my_module/upload", type="http", auth="user", methods=["POST"], csrf=False)
