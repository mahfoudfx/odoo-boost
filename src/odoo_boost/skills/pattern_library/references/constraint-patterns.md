...
class MyModel(models.Model):
    ...
    _sql_constraints = [
        ...
        # Date constraint
        ("dates_check", "CHECK(date_end >= date_start)", "End date must be after start date."),
        # Not null with condition
        (
...
def _check_code_unique(self):
    ...
    for record in self:
        ...
        if duplicate:
            raise ValidationError(f"Code '{record.code}' already exists in this company.")
```
...
def _check_phone_format(self):
    ...
    for record in self:
        if record.phone and not re.match(phone_pattern, record.phone):
            raise ValidationError(f"Invalid phone format: {record.phone}")
```
...
def _check_quantity_available(self):
    ...
    for record in self:
        ...
        if record.quantity > available:
            raise ValidationError(
                f"Requested quantity ({record.quantity}) exceeds available stock ({available})."
            )
```
    ...
    for record in self:
        if record.state == "confirmed":
            if not record.partner_id:
                raise ValidationError("Partner is required for confirmed records.")
            if not record.line_ids:
                raise ValidationError("Lines are required for confirmed records.")
```
...
def _check_total_percentage(self):
    ...
    for record in self:
        ...
        if abs(total - 100) > 0.01:  # Allow small rounding errors
            raise ValidationError(f"Percentages must sum to 100%. Current total: {total}%")
```
...
def _check_fiscal_period(self):
    ...
    for record in self:
        ...
        if not period:
            raise ValidationError(f"No fiscal year defined for date {record.date}.")

        if period.state == "closed":
            raise ValidationError(f"Cannot post to closed fiscal period: {period.name}")
```
