...
_logger.info("Order %s: state changed from %s to %s", self.name, old_state, new_state)
...
class PerformanceLogger:
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        if duration > 1.0:  # Log slow operations
            self.logger.warning("Slow operation: %s took %.2fs", self.operation_name, duration)
        else:
            self.logger.debug("Operation %s completed in %.3fs", self.operation_name, duration)
...
def _log_query_count(self, operation_name):
    ...
    if query_count > 100:
        _logger.warning("High query count in %s: %d queries", operation_name, query_count)
```
...
class RequestLogger(http.Controller):
    ...
    def my_endpoint(self, **kwargs):
        ...
        _logger.info("[%s] Request: user=%s, params=%s", request_id, request.env.user.login, kwargs)

        try:
            result = self._process_request(**kwargs)

            duration = time.time() - start_time
            _logger.info("[%s] Response: success, duration=%.3fs", request_id, duration)

            return result

        except Exception as e:
            duration = time.time() - start_time
            _logger.error("[%s] Response: error=%s, duration=%.3fs", request_id, str(e), duration)
            raise
