    ...
    def create(self, vals_list):
        for vals in vals_list:
            if vals.get("x_employee_number", "New") == "New":
                vals["x_employee_number"] = (
                    self.env["ir.sequence"].next_by_code("hr.employee.number") or "New"
                )
    ...
    def _check_leave_requirements(self):
        for leave in self:
            ...
            if leave_type.x_requires_attachment and not leave.x_attachment_ids:
                raise ValidationError(f"Attachment required for {leave_type.name}.")
```
...
class HrEmployee(models.Model):
    ...
    def _compute_onboarding_progress(self):
        for employee in self:
            ...
            done = len(employee.x_onboarding_ids.filtered(lambda t: t.state == "done"))
