...
class EquipmentAsset(models.Model):
    ...
    def _compute_is_under_warranty(self) -> None:
        ...
        for record in self:
            record.is_under_warranty = record.warranty_expiry and record.warranty_expiry >= today
    ...
    def create(self, vals_list: list[dict[str, Any]]) -> "EquipmentAsset":
        ...
        for vals in vals_list:
            if not vals.get("name"):
                vals["name"] = self.env["ir.sequence"].next_by_code("equipment.asset") or _(
                    "New Asset"
                )
    ...
    def unlink(self) -> bool:
        ...
        for record in self:
            ...
            if active_maintenance:
                raise UserError(_("Cannot delete asset with active maintenance requests."))
        return super().unlink()
