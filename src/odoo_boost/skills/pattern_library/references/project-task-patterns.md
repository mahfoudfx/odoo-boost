...
class ProjectProject(models.Model):
    ...
    def _compute_hours(self):
        for project in self:
            project.x_total_hours = sum(project.task_ids.mapped("effective_hours"))
...
class ProjectTask(models.Model):
    ...
    def _compute_cost(self):
        for task in self:
            ...
            cost = sum(ts.unit_amount * ts.employee_id.hourly_cost for ts in task.timesheet_ids)
            task.x_cost = cost
...
class ProjectTask(models.Model):
    ...
    def _compute_is_blocked(self):
        ...
        for task in self:
            blocking = task.x_depends_on_ids.filtered(lambda t: t.stage_id not in done_stages)
