...
class WebsiteSaleExtend(WebsiteSale):
    ...
    def cart(self, **post):
        ...
        if hasattr(response, "qcontext"):
            response.qcontext["x_loyalty_points"] = request.env.user.partner_id.x_loyalty_points
...
def check_available_views(self, model_name):
    ...
    views = self.env["ir.ui.view"].search([("model", "=", model_name), ("type", "!=", False)])
...
def _check_view_type_exists(self, model_name, view_type):
    ...
    return self.env["ir.ui.view"].search(
        [("model", "=", model_name), ("type", "=", view_type)], limit=1
    )
