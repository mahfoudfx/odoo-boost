...
class MyPortal(CustomerPortal):
    ...
    @http.route(
        ["/my/documents", "/my/documents/page/<int:page>"], type="http", auth="user", website=True
    )
    ...
    def portal_document_detail(self, document_id, access_token=None, **kw):
        ...
        try:
            document_sudo = self._document_check_access("my.document", document_id, access_token)
        except AccessError:
            return request.redirect("/my")

        return request.render(
            "my_module.portal_document_detail",
            {
    ...
    def _document_check_access(self, model, document_id, access_token=None):
        ...
        if access_token:
            ...
            if not document_sudo.access_token or document_sudo.access_token != access_token:
    ...
    def _compute_share_url(self):
        ...
        for record in self:
            if record.access_token:
                record.share_url = (
                    f"{base_url}/my/documents/{record.id}?access_token={record.access_token}"
                )
            else:
...
class PublicDocumentController(http.Controller):
    @http.route("/document/view/<string:token>", type="http", auth="public", website=True)
    def view_document_public(self, token, **kw):
        """View document with access token (no login required)."""
        document = (
            request.env["my.document"]
            .sudo()
            .search(
                [
                    ("access_token", "=", token),
        ...
        return request.render(
            "my_module.document_public_view",
            {
                "document": document,
            },
        )

    @http.route("/document/download/<string:token>", type="http", auth="public")
...
class SignedUrlMixin(models.AbstractModel):
    ...
    def _get_signing_key(self):
        ...
        return (
            ...
            .sudo()
            .get_param("my_module.signing_key", default="default-secret-key")
        )
    ...
    def generate_signed_url(self, base_path, params, validity_seconds=3600):
        ...
        signature = hmac.new(
            self._get_signing_key().encode(), message.encode(), hashlib.sha256
        ).hexdigest()
