...
class EquipmentItem(models.Model):
    ...
    def action_return(self) -> bool:
        ...
        for record in self:
            ...
            current_assignment = record.assignment_ids.filtered(lambda a: not a.date_returned)
    ...
    def action_retire(self) -> bool:
        ...
        for record in self:
            if record.state == "assigned":
                raise UserError(
                    f"Cannot retire assigned equipment '{record.name}'. Return it first."
                )
...
class TestEquipmentItem(TestEquipmentCommon):
    ...
    def test_assign_equipment(self):
        ...
        self.assertEqual(self.equipment.assignment_ids[0].employee_id, self.employee)
    ...
    def test_multi_company_isolation(self):
        ...
        visible = self.env["equipment.item"].with_user(self.user_company2).search([])
